...
class _LoadingWidgetState extends State<LoadingWidget> with TickerProviderStateMixin {
  ...
  late AnimationController _outerBreathingController; // For breathing effect on inner circle and nodes
...
class LoadingPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (breathingValue != null) {
      final double innerCircleScale = 0.2 + (0.2 * breathingValue!); // Scales between 0.2x and 0.4x of baseRadius
      final double innerCircleRadius = baseRadius * innerCircleScale;
      canvas.drawCircle(center, innerCircleRadius, strokePaint); // Changed to stroke instead of fill
    } else {
  ...
  void _drawArcWithNode(Canvas canvas, Offset center, double radius, double strokeWidth, Paint strokePaint,
      Paint fillPaint, double startAngle, double sweepAngle, double nodeRadius) {
    // Draw the arc
    canvas.drawArc(
    ...
    if (outerBreathingValue != null) {
      final double breathingScale = 0.5 + (0.7 * outerBreathingValue!); // Scales between 0.9x and 1.0x
      canvas.drawCircle(
        nodeCenter,
  ...
  bool shouldRepaint(covariant LoadingPainter oldDelegate) {
    return oldDelegate.rotationValue != rotationValue || oldDelegate.breathingValue != breathingValue;
  }
}
